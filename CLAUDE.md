...
- **Dependency overrides**: The root pubspec overrides `test_api` and `meta` for workspace resolution — don't remove those unless you've verified the analyzer/flutter_test conflict is gone.
...
- **`dart test` limitation**: pure-Dart packages should work via `dart test` in principle, but a workspace-level `test_api` version mismatch currently makes `dart test` fail in `fledge_ecs` and `fledge_net`. Use `flutter test` locally; CI's `melos run test` wrapper routes correctly.
