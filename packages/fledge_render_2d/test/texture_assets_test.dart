...
void main() {
  ...
  group('CanvasSpriteDrawer resolves Handle<Texture>', () {
    test(
      'a Handle<Texture> drives drawSpriteBatch through the atlas path',
      () async {
    ...
    test(
      'legacy registerTexture still resolves (backwards compatibility)',
      () async {
        final app = App()..addPlugin(RenderPlugin());
        final drawer =
            app.world.getResource<SpriteDrawer>()! as CanvasSpriteDrawer;
        ...
        drawer.registerTexture(
          legacy,
          createSolidColorImageSync(const Color(0xFF00FF00)),
        );
    ...
    test('Assets<Texture>.add path takes precedence over legacy map', () async {
      ...
      final texture = Texture(
        createSolidColorImageSync(const Color(0xFFAA0000)),
        1,
        1,
      );
